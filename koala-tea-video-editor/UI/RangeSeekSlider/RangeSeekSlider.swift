...
@IBDesignable open class RangeSeekSlider: UIControl {
    ...
    @IBInspectable open var labelPadding: CGFloat = 4.0 {
        didSet {
            updateLabelPositions()
        }
    }

    open var labelWidth: CGFloat = 40 {
        didSet {
            updateLabelPositions()
        }
    }
    
    open var labelHeight: CGFloat = 20 {
        didSet {
            updateLabelPositions()
        }
    }
    ...
    fileprivate let minLabel: UILabel = UILabel()
    fileprivate let maxLabel: UILabel = UILabel()
    ...
    private var leftHandleAccessibilityElement: UIAccessibilityElement {
        ...
        element.accessibilityValue = minLabel.text
    ...
    private var rightHandleAccessibilityElement: UIAccessibilityElement {
        ...
        element.accessibilityValue = maxLabel.text
...
//        let handle: CALayer = (handleTracking == .left) ? leftHandle : rightHandle
        ...
        if handleTracking == .left {
            self.bringSubview(toFront: minLabel)
        }
        if handleTracking == .right {
            self.bringSubview(toFront: maxLabel)
        }
...
//        animate(handle: handle, selected: false)
    ...
    private func setup() {
        ...
        let labelFrame: CGRect = CGRect(x: 0.0, y: 0.0, width: labelWidth, height: labelHeight)

        minLabel.isUserInteractionEnabled = false
        minLabel.backgroundColor = .lightGray
//        minLabel.layer.opacity = 0.5
        minLabel.textAlignment = .center
        minLabel.isHidden = true
        minLabel.cornerRadius = 8
        ...
        maxLabel.backgroundColor = .lightGray
//        maxLabel.layer.opacity = 0.5
        maxLabel.textAlignment = .center
        maxLabel.isHidden = true
        maxLabel.cornerRadius = 8
    ...
    private func updateLineHeight() {
        ...
        let sliderLineHeight = lineHeight / 3
        sliderLine.frame = CGRect(x: lineLeftSide.x,
                                  y: self.frame.height / 2 - sliderLineHeight / 2,
        ...
        leftHandle.frame = CGRect(x: 0.0, y: 0.0, width: handleDiameter, height: lineHeight)
        rightHandle.frame = CGRect(x: 0.0, y: 0.0, width: handleDiameter, height: lineHeight)
    }
...
//        maxLabel.isHidden = hideLabels
        ...
        if let replacedString = delegate?.rangeSeekSlider(self, stringForMinValue: selectedMinValue) {
            minLabel.text = replacedString
        } else {
            minLabel.text = numberFormatter.string(from: selectedMinValue as NSNumber)
        }

        if let replacedString = delegate?.rangeSeekSlider(self, stringForMaxValue: selectedMaxValue) {
            maxLabel.text = replacedString
        } else {
            maxLabel.text = numberFormatter.string(from: selectedMaxValue as NSNumber)
        }

        if let nsstring = minLabel.text as? NSString {
            minLabelTextSize = nsstring.size(withAttributes: [NSAttributedStringKey.font: minLabelFont])
        }

        if let nsstring = maxLabel.text as? NSString {
    ...
    private func updateColors() {
        ...
        if let initialColor = initialColor, isInitial {
            minLabel.textColor = initialColor
            maxLabel.textColor = initialColor
            sliderLineBetweenHandles.backgroundColor = initialColor.cgColor
            sliderLine.backgroundColor = initialColor.cgColor

            let rightColor: CGColor = (rightHandleImage == nil) ? initialColor.cgColor : UIColor.clear.cgColor
            rightHandle.backgroundColor = rightColor
            rightHandle.borderColor = rightColor
            let leftColor: CGColor = (leftHandleImage == nil) ? initialColor.cgColor : UIColor.clear.cgColor
            leftHandle.backgroundColor = leftColor
            leftHandle.borderColor = leftColor
        } else {
            let tintCGColor: CGColor = tintColor.cgColor
            minLabel.textColor = minLabelColor ?? tintColor
            maxLabel.textColor = maxLabelColor ?? tintColor
            sliderLineBetweenHandles.backgroundColor = colorBetweenHandles?.cgColor ?? tintCGColor
            sliderLine.backgroundColor = tintCGColor

            var rightColor: CGColor = handleColor?.cgColor ?? tintCGColor
            var leftColor: CGColor = handleColor?.cgColor ?? tintCGColor
            if rightHandleImage != nil {
                rightColor = UIColor.clear.cgColor
            }
            if leftHandleImage != nil {
    ...
    private func updateHandlePositions() {
        ...
        sliderLineBetweenHandles.frame = CGRect(x: leftHandle.position.x,
                                                y: sliderLine.frame.midY - lineHeight / 2,
                                                width: rightHandle.position.x - leftHandle.position.x,
                                                height: lineHeight)

        minLabel.frame = CGRect(x: leftHandle.frame.center.x - (labelWidth / 2),
                                y: leftHandle.frame.minY - labelHeight - 4.0,
                                width: labelWidth,
                                height: labelHeight)
        maxLabel.frame = CGRect(x: rightHandle.frame.center.x - (labelWidth / 2),
                                y: rightHandle.frame.minY - labelHeight - 4.0,
                                width: labelWidth,
                                height: labelHeight)
    }
...
private final class RangeSeekSliderLeftElement: UIAccessibilityElement {
    ...
    override func accessibilityIncrement() {
        ...
        accessibilityValue = slider.minLabel.text as? String
    }
...
private final class RangeSeekSliderRightElement: UIAccessibilityElement {
    ...
    override func accessibilityIncrement() {
        ...
        slider.refresh()
        accessibilityValue = slider.maxLabel.text as? String
    }
